...
namespace DancingLineFanmade.Level
{
    ...
    [Obsolete]
    public class OldCameraFollower : MonoBehaviour
    {
        private Transform selfTransform;

        public static OldCameraFollower Instance { get; private set; }
            ...
            if (LevelManager.GameState == GameStatus.Playing && follow)
            {
                if (smooth)
                    selfTransform.Translate(new Vector3(translation.x * followSpeed.x * Time.deltaTime,
                        translation.y * followSpeed.y * Time.deltaTime,
                        translation.z * followSpeed.z * Time.deltaTime));
                else selfTransform.position = target.position;
            }
        }
    ...
    [Serializable]
    public class OldCameraSettings
    {
        ...
        internal OldCameraSettings GetCamera()
        {
            var settings = new OldCameraSettings();
