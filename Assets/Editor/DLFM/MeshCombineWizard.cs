...
public class MeshCombineWizard : EditorWindow
{
	public GameObject combineParent;
	public string resultPath;
	bool is32bit = true;
	bool generateSecondaryUVs = false;

	[MenuItem("Tools/DLFM/Mesh Combine Wizard")]
	public static void Init()
	{
		GetWindow<MeshCombineWizard>().Show();
	}

	public void OnGUI()
	{
		combineParent = (GameObject)EditorGUILayout.ObjectField("Parent of objects to combine", combineParent, typeof(GameObject));
		resultPath = EditorGUILayout.TextField("Result Path", resultPath);
		is32bit = EditorGUILayout.Toggle("32-bit Indexes", is32bit);
